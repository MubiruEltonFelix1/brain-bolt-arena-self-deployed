...
begin
  ...
  if v_order is not null and jsonb_typeof(v_order) = 'array' and jsonb_array_length(v_order) > 0 then
    select ordered.question_id into v_expected_qid
    from (
      select value::uuid as question_id, ordinality::int as sort_order
      from jsonb_array_elements_text(v_order) with ordinality
    ) ordered
    where not exists (
      select 1
      from public.answers a
      where a.participant_id = p_participant_id
        and a.question_id = ordered.question_id
    )
  ...
  if v_expected_qid is null then
    raise exception 'Quiz already completed';
  end if;

  if v_expected_qid <> p_question_id then
