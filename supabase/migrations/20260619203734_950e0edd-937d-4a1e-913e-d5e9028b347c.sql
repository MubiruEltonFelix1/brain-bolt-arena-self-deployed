...
begin
  ...
  select s.status, s.question_order, s.quiz_id, q.time_per_question
    into v_status, v_order, v_quiz_id, v_quiz_default
  ...
  if v_order is not null and jsonb_typeof(v_order) = 'array' and jsonb_array_length(v_order) > 0 then
    select exists (
      select 1
      from jsonb_array_elements_text(v_order) ordered(question_id)
      where ordered.question_id::uuid = p_question_id
    ) into v_question_allowed;
  else
    select exists (
      select 1
  ...
  if not coalesce(v_question_allowed, false) then
    raise exception 'Question is not in this session';
  end if;
