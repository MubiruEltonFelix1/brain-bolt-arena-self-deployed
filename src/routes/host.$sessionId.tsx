...
function HostScreen({ onConn }: { onConn: (c: ConnInfo) => void }) {
  ...
  const [roundStats, setRoundStats] = useState<
    Array<{ selected_index: number; vote_count: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      if (document.visibilityState === "visible") syncServerClock(true);
    };
  ...
  const load = useCallback(async () => {
    const { data: s, error: sErr } = await supabase
      .from("sessions")
      .select("*, quiz:quizzes(title,time_per_question)")
      .eq("id", sessionId)
      .maybeSingle();
    if (sErr) {
      setLoadFailed(true);
    ...
    const { data: qs } = await supabase
      .from("questions")
      .select(
        "id,text,options,correct_index,position,time_limit_sec,point_value,question_type,image_url,double_points,is_playable,correct_lat,correct_lng,correct_number,number_min,number_max,max_distance_km,accepted_answers,reveal_stages,audio_url,geo_region_label",
      )
    ...
    const { data: ps } = await supabase
      .from("participants")
      .select("id,nickname,score,team_id,avatar_id")
    ...
    const order =
      (s as unknown as Session).question_order ?? (qs as Question[] | null)?.map((q) => q.id) ?? [];
    ...
    setup: (ch) =>
      ch
        .on(
          ...
          (payload) => {
            setSession(
              (prev) => ({ ...(prev as Session), ...(payload.new as Partial<Session>) }) as Session,
            );
          },
        )
        .on(
  ...
  const currentQuestion =
    currentIndex >= 0 ? questions.find((q) => q.id === orderedIds[currentIndex]) : null;
  ...
  const answeredThisRound = currentQuestion
    ? new Set(
        answersForRound
    ...
    : 0;
  const correctThisRound = currentQuestion
    ? answersForRound.filter((a) => a.question_id === currentQuestion.id && a.is_correct).length
    : 0;
  ...
  async function startGame() {
    ...
    await runControl(async () => {
      ...
      if (
        ...
        (Array.isArray(session.question_order) && session.question_order.length === 0)
      ) {
        const nonFeedback = playable.filter((q) => q.question_type !== "feedback");
        const feedback = playable.filter((q) => q.question_type === "feedback");
  ...
  async function nextRound() {
    await runControl(async () => {
      ...
      if (error) {
        toastError(error, { context: "nextRound (advance_question)" });
        return;
      }
  ...
  async function doSkipQuestion() {
    ...
    await runControl(async () => {
      ...
      if (error) {
        toastError(error, { context: "doSkipQuestion (skip_current_question)" });
        return;
      }
      const row = Array.isArray(data) ? data[0] : data;
  ...
  return (
    <HostShell title="Host">
          ...
          <div className="space-y-4">
            ...
            {session.status === "active" && currentQuestion && !revealed && !inIntro && (
              <div className="bg-card border border-border p-8 relative overflow-hidden">
                ...
                <div
                  className="absolute top-0 left-0 h-1 bg-volt"
                  style={{
                    width: `${(remaining / timeLimitMs) * 100}%`,
                ...
                <div className="flex justify-between items-center pt-2 mb-6">
                  ...
                  {currentQuestion.double_points && (
                    <span className="font-mono text-xs uppercase text-amber-spark">
                      ⚡ DOUBLE POINTS
                    </span>
                  )}
                ...
                {currentQuestion.image_url && currentQuestion.question_type !== "image_reveal" && (
                  <img
                    ...
                    alt={`Illustration for the question: ${currentQuestion.text}`}
                    className="w-full max-h-64 object-contain mb-4 bg-background border border-border"
                  />
                )}
                {currentQuestion.question_type === "image_reveal" &&
                  ...
                  (() => {
                    ...
                    const stageIdx = Math.min(
                      stages - 1,
                      Math.floor((elapsedMs / Math.max(timeLimitMs, 1)) * stages),
                    );
                    ...
                    return (
                      <div className="mb-4 border border-border bg-background">
                        <div className="relative w-full aspect-video overflow-hidden">
                          <img
                            src={currentQuestion.image_url}
                            alt={`Progressively revealed image for the question: ${currentQuestion.text}`}
                        ...
                        <div className="px-3 py-2 border-t border-border flex items-center justify-between font-mono text-[10px] uppercase text-foreground/60">
                          <span>
                            🖼️ Reveal stage {stageIdx + 1} / {stages}
                          </span>
                          <span className="text-volt">
                            {answeredThisRound}/{participants.length} submitted
                          </span>
                        </div>
                  ...
                  <div className="mb-4 border border-border bg-background p-3 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-mono text-[10px] uppercase text-foreground/60">
                        Audio question — players hear the clip one time
                      </p>
                      <audio controls src={currentQuestion.audio_url} className="w-full mt-1" />
                    </div>
                  </div>
                )}
                <h3 className="font-display text-3xl uppercase leading-tight mb-6">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {currentQuestion.question_type === "true_false" ? (
                    ...
                    currentQuestion.options.map((opt, i) => (
                      <div
                        key={i}
                        className="p-4 border border-border bg-background flex items-center gap-3"
                      >
            ...
            <div className="bg-card border border-border p-4">
              ...
              <div className="space-y-1 max-h-[400px] overflow-auto">
                {participants.length === 0 && (
                  <p className="text-foreground/40 text-xs font-mono">No players yet</p>
                )}
                {participants.map((p, i) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="flex items-center gap-2 py-1.5 px-2 bg-background/40"
                    >
      ...
      {confirmEndEarly && currentQuestion && (
        ...
        >
          <p className="text-sm text-foreground/80">
            ...
            <span className="text-pink-shock font-bold">
              {Math.max(0, participants.length - answeredThisRound)}
            </span>{" "}
            · Time left: <span className="text-volt font-bold">{Math.max(0, remainingSec)}s</span>
          </p>
          <p className="text-xs text-foreground/60">
            Players who have not submitted will be treated exactly like timer expiry — no answer
            recorded, no points awarded. Submitted answers are still scored normally.
          </p>
        </ConfirmModal>
      )}
      {confirmSkip && currentQuestion && (
        ...
        >
          <p className="text-sm text-foreground/80">
            All {answeredThisRound} submitted answer{answeredThisRound === 1 ? "" : "s"} for this
            question will be voided, and any points already awarded for it will be refunded.
            Unanswered players are not penalised.
          </p>
          <p className="text-xs text-foreground/60">
            The question stays in the session record marked as skipped, so it appears in future
            reporting.
          </p>
        </ConfirmModal>
      )}
        ...
        <div className="border-2 border-volt bg-volt/5 p-6 text-center space-y-2">
          ...
          {isMap ? (
            <p className="font-display text-3xl italic text-volt">
              🗺️{" "}
              {question.geo_region_label ??
                (question.correct_lat != null
                  ? `${Number(question.correct_lat).toFixed(3)}, ${Number(question.correct_lng).toFixed(3)}`
                  : "—")}
            </p>
          ) : isNum ? (
            <p className="font-display text-4xl italic text-volt">
              🎯{" "}
              {question.correct_number != null
                ? Number(question.correct_number).toLocaleString()
                : "—"}
            </p>
          ) : (
            <TextAnswerReveal
          ...
          <p className="font-mono text-xs text-foreground/60">
            {totalVotes} of {totalVotes} players submitted
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          <p className="font-mono text-[10px] uppercase text-foreground/60">
            Answer distribution · {totalVotes} votes
          </p>
          {labels.map((label, i) => {
              ...
              <div key={i} className={`border ${isCorrect ? "border-volt" : "border-border"} p-2`}>
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className={`font-medium text-sm ${isCorrect ? "text-volt" : ""}`}>
                    {isCorrect ? "✓ " : ""}
                    {isTF ? label : `${["A", "B", "C", "D", "E", "F"][i]} — ${label}`}
                  </span>
                ...
                <div className="h-2 bg-border relative overflow-hidden">
                  <div
                    className={`absolute inset-y-0 left-0 ${isCorrect ? "bg-volt" : "bg-cyan-jolt/60"}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
          ...
          return (
            <div key={p.id} className="flex items-center gap-2 py-1.5 px-2 bg-background/40">
              ...
              {diff !== 0 && prev !== undefined && (
                <span
                  className={`font-mono text-[10px] ${diff > 0 ? "text-volt" : "text-pink-shock"}`}
                >
...
function PodiumCard({
  ...
  return (
    <div
      ...
      style={{ animation: `podium-rise 0.7s cubic-bezier(0.32,0.72,0,1) ${delay} both` }}
    >
      <div className="relative">
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 30px color-mix(in oklab, ${colorVar} 40%, transparent)`,
          }}
        >
          <PlayerAvatar
        ...
        <div
          ...
          style={{
            background: colorVar,
            animation: `badge-pop 0.5s cubic-bezier(0.32,0.72,0,1) ${delay} both`,
          }}
        >
          #{place}
        </div>
      ...
      <div className="text-center max-w-full px-2">
        ...
        <p className="font-display text-2xl md:text-3xl italic" style={{ color: colorVar }}>
          {p.score.toLocaleString()}
        </p>
      </div>
      <div
        ...
        style={{
          height,
          background: `linear-gradient(180deg, color-mix(in oklab, ${colorVar} 18%, transparent), color-mix(in oklab, ${colorVar} 4%, transparent))`,
          borderColor: colorVar,
          boxShadow: `inset 0 -20px 40px color-mix(in oklab, ${colorVar} 15%, transparent)`,
        }}
      >
        <span className="font-display text-4xl md:text-6xl italic" style={{ color: colorVar }}>
          {medal}
        </span>
      </div>
    </div>
          ...
          <div className="max-h-[300px] overflow-auto">
            {rest.map((p, i) => (
              ...
              >
                ...
                <span className="font-display text-base italic text-volt text-right">
                  {p.score.toLocaleString()}
                </span>
              </div>
            ))}
    ...
    <div className="border border-border bg-background/40 p-4 space-y-3 text-left max-h-[400px] overflow-auto">
      ...
      {ordered.map((q, i) => (
        <div key={q.id} className="border border-border p-3">
          <p className="font-mono text-[10px] text-foreground/40 mb-1">
              ...
              <span className="text-volt">
                {q.question_type === "true_false"
                  ...
                  : q.question_type === "map_pin"
                    ? (q.geo_region_label ??
                      (q.correct_lat != null
                        ? `🗺️ ${Number(q.correct_lat).toFixed(3)}, ${Number(q.correct_lng).toFixed(3)}`
                        : "—"))
                    : q.question_type === "number"
                      ? q.correct_number != null
                        ? `🎯 ${Number(q.correct_number).toLocaleString()}`
                        : "—"
                      : q.question_type === "type"
                        ? q.accepted_answers?.[0]
                          ? `⌨️ ${q.accepted_answers[0]}`
                          : "—"
...
function TextAnswerReveal({
  ...
  const [groups, setGroups] = useState<
    Array<{ key: string; label: string; count: number; correct: boolean }>
  >([]);
  ...
  return (
    <div className="space-y-3">
      ...
      {phase !== "loading" && groups.length > 0 && (
        <div className="relative">
          <div className={`grid gap-3 ${phase === "reveal" ? "sm:grid-cols-1" : "sm:grid-cols-2"}`}>
            {groups.map((g, i) => {
              ...
              return (
                ...
                >
                  <span className="font-bold text-lg md:text-xl truncate flex items-center gap-2">
                    {phase === "reveal" && isCorrect && <span className="text-volt">✓</span>}
                    {g.label}
                  </span>
                  <span
...
function FeedbackReveal({ questionId }: { questionId: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {loading && <p className="font-mono text-xs text-foreground/50 p-4">Loading responses…</p>}
...
function OrderingReveal({ question }: { question: Question }) {
    ...
    (async () => {
      ...
      const parsed = (data ?? [])
        ...
        .map((v) => ({
          ...
          correct_positions: Number(v.correct_positions ?? 0),
        }));
      setRows(parsed);
      setLoading(false);
    })();
  ...
  const positionCorrect = question.options.map(
    (_, i) => rows.filter((r) => r.order[i] === i).length,
  );
          ...
          {question.options.map((label, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-mono text-[10px] text-foreground/60 shrink-0">
                  {positionCorrect[i]}/{rows.length} · {pct}%
                </span>
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="border border-border p-3 text-center">
          ...
          <p className="font-display text-2xl italic text-volt">
            {avg.toFixed(1)}
            <span className="text-foreground/40 text-sm">/{total}</span>
          </p>
        </div>
      </div>
    </div>
